...
local function build()
 local p=call('GetVisualPolicy') or {};local c=call('GetCompatibility') or {};local f=call('GetFallbackState') or {};local a=call('GetVisualApplyState') or {};local b=call('GetVisualBudget') or {};local ba=call('GetVisualBudgetAdapter') or {};local ctx=call('GetVisualContextBridge') or {};local fin=call('GetVisualFinishState') or {};local tr=call('GetVisualTransitionState') or {};local wf=call('GetWeatherFinishState') or {};local md=call('GetMasterDirectorState') or {};local sc=call('GetVisualSceneState') or {};local co=call('GetVisualComposerState') or {};local pr=call('GetVisualProfileDirectorState') or {};local integ=call('GetRuntimeIntegrity') or {};local prof=call('GetActiveProfile')
 return {phase=p.phase or md.phase or sc.phase or '?',weather=p.weather or wf.weather or md.weather or sc.weather or '?',wet=p.wet==true or (tonumber(wf.wet) or 0)>0.5,interior=p.interior==true,emergency=p.emergency==true,profile=p.profile or md.profile or '?',requestedProfile=prof or '?',recommendedProfile=pr.recommended or '?',recommendationReason=pr.reason or '?',manualProfile=call('IsProfileManual')==true,visualSettings=c.visualSettings==true,timecycle=c.timecycle==true,fallback=f.mode or '?',fallbackReason=f.reason or '?',applyEnabled=a.enabled==true,budgetTier=b.tier or '?',budgetMultiplier=b.multiplier or 1.0,adapterMultiplier=ba.multiplier or 1.0,finish=fin.finish or wf.finish or 0.0,night=fin.night or wf.night or md.night or sc.night or 0.0,wetFactor=fin.wet or wf.wet or md.wetFactor or sc.wet or 0.0,stormFactor=fin.storm or wf.storm or md.storm or sc.storm or 0.0,fog=wf.fog or sc.fog or 0.0,overcast=wf.overcast or sc.overcast or 0.0,sky=wf.sky or 0.0,reflection=wf.reflection or co.reflection or 0.0,contextIntensity=ctx.intensity or 0.0,transitionProgress=tr.progress or 0.0,transitionFrom=tr.from or '?',transitionTo=tr.to or '?',masterQuality=md.quality or 1.0,composerQuality=co.quality or 1.0,sceneStreet=sc.streetDemand or 0.0,sceneAtmosphere=sc.atmosphereDemand or 0.0,sceneCinematic=sc.cinematic or 0.0,integrity=integ.state or '?',integrityOk=integ.ok or 0,integrityTotal=integ.total or 0,frameMs=b.frameMs or 0,lastState=a.lastStateKey or ''}
end
local function printStatus()
 local s=build();Status.last=s
 print('[SilvaVision] ===== MEGA ULTRA STATUS V20 =====')
 ...
 print(('[SilvaVision] composer q=%.2f night=%.2f wet=%.2f storm=%.2f reflection=%.2f'):format(s.composerQuality,s.night,s.wetFactor,s.stormFactor,s.reflection))
