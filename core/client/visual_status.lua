...
local function printStatus()
 ...
 print(('[SilvaVision] solar=%.2f golden=%.2f profile=%s requested=%s recommended=%s (%s) manual=%s'):format(s.solar,s.golden,s.profile,s.requestedProfile,s.recommendedProfile,s.recommendationReason,tostring(s.manualProfile)))
 ...
 print(('[SilvaVision] composer q=%.2f night=%.2f wet=%.2f storm=%.2f refl=%.2f emergency=%.2f'):format(s.composerQuality,s.night,s.wetFactor,s.stormFactor,s.reflection,s.emergencySignal))
 print(('[SilvaVision] scene street=%.2f atmosphere=%.2f cinematic=%.2f'):format(s.sceneStreet,s.sceneAtmosphere,s.sceneCinematic))
...
CreateThread(function()Wait(10000);if Status.enabled then printStatus()end end)
