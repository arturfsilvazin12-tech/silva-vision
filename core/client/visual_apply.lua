...
-- Final Fusion agora alimenta os alvos visuais, sempre com limites seguros.
...
local function apply()
 ...
 local s=call('GetVisualPolicy') or {};local scene=call('GetVisualSceneState') or {};local comp=call('GetVisualComposerState') or {};local fusion=call('GetVisualFinalFusionState') or {};local profiles=call('GetProfiles') or {}
 ...
 local nightF=clamp(num(fusion.night,num(comp.night,num(finish.night,num(scene.night,num(ctx.night,0))))),0,1);local wetF=clamp(num(fusion.wet,num(comp.wet,num(finish.wet,num(scene.wet,num(wf.wet,0))))),0,1);local stormF=clamp(num(fusion.storm,num(comp.storm,num(finish.storm,num(scene.storm,num(wf.storm,0))))),0,1);local reflectionF=clamp(num(fusion.reflection,num(comp.reflection,num(wf.reflection,num(scene.reflectionDemand,0)))),0,1);local sceneLight=clamp(num(fusion.street,num(comp.street,num(scene.streetDemand,num(lb.street,0)))),0,1);local emergencyF=clamp(num(fusion.emergency,num(comp.emergency,num(e.active,num(finish.emergency,0)))),0,1)
 ...
 local fusionSignal=clamp(num(fusion.signal,0),0,1);local fusionPriority=clamp(num(fusion.priority,0),0,1);local fusionContrast=clamp(num(fusion.contrast,1),1,1.13);local fusionSurface=clamp(num(fusion.surface,0),0,1)
 ...
 local reflection=clamp(num(p.distantReflection,0.85)*lightWeight+wetF*0.10+stormF*0.02+reflectionF*0.06+vehicleWet*0.02+fusionSignal*0.05+fusionPriority*0.03+fusionSurface*0.02,0.70,1.00)
 local upscale=clamp(num(p.distantUpscale,1.50)*lightWeight+nightF*0.10+reflectionF*0.04+fusionSignal*0.03,1.10,1.80)
 valid('distantlights.size',distant);valid('distantlights.sizeReflections',reflection);valid('distantlights.sizeUpscale',upscale);valid('distantlights.sizeUpscaleReflections',clamp(upscale+wetF*0.05+reflectionF*0.03+fusionSignal*0.02,1.10,1.80))
 local rain=num(p.rain,3500)+stormF*650+clamp(num(fx.rain,0),0,1)*250+fusionSurface*120;valid('rain.NumberParticles',clamp(rain*rainWeight,2800,5500));valid('rain.UseLitShader',1.0);valid('rain.diffuse',clamp(0.90+wetF*0.10+stormF*0.02+fusionContrast*0.01-0.01,0.90,1));valid('rain.ambient',clamp(0.35+wetF*(0.08+stormF*0.03),0.35,0.46));valid('rain.wrapScale',0.60);valid('rain.wrapBias',0.40);valid('rain.defaultlight.red',1.0);valid('rain.defaultlight.green',1.0);valid('rain.defaultlight.blue',1.0)
 ...
 Apply.lastProfile=name;Apply.lastMultiplier=mult;Apply.lastStateKey=table.concat({scene.phase or s.phase or '?',scene.weather or s.weather or '?',string.format('night%.2f',nightF),string.format('urban%.2f',nightUrban),string.format('wet%.2f',wetF),string.format('solar%.2f',solar),string.format('em%.2f',emergencyF),string.format('fusion%.2f',fusionSignal),string.format('surface%.2f',fusionSurface),name,budget.tier or '?'},'|');return true
end
