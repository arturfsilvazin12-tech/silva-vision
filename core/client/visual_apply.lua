...
-- Rain/Wet/Emergency Director agora reforça a resposta visual dentro de limites seguros.
...
local function apply()
 ...
 local s=call('GetVisualPolicy') or {};local scene=call('GetVisualSceneState') or {};local comp=call('GetVisualComposerState') or {};local fusion=call('GetVisualFinalFusionState') or {};local rainDir=call('GetVisualRainSurfaceDirectorState') or {};local profiles=call('GetProfiles') or {}
 ...
 local nightF=clamp(num(fusion.night,num(comp.night,num(finish.night,num(scene.night,num(ctx.night,0))))),0,1);local wetF=clamp(math.max(num(fusion.wet,num(comp.wet,num(finish.wet,num(scene.wet,num(wf.wet,0))))),num(rainDir.wet,0)),0,1);local stormF=clamp(math.max(num(fusion.storm,num(comp.storm,num(finish.storm,num(scene.storm,num(wf.storm,0))))),num(rainDir.storm,0)),0,1);local reflectionF=clamp(math.max(num(fusion.reflection,num(comp.reflection,num(wf.reflection,num(scene.reflectionDemand,0)))),num(rainDir.reflection,0)),0,1);local sceneLight=clamp(num(fusion.street,num(comp.street,num(scene.streetDemand,num(lb.street,0)))),0,1);local emergencyF=clamp(math.max(num(fusion.emergency,num(comp.emergency,num(e.active,num(finish.emergency,0)))),num(rainDir.emergency,0)),0,1)
 local rainF=clamp(math.max(num(fusion.rain,0),num(rainDir.rain,0)),0,1);local puddleF=clamp(num(rainDir.puddle,num(fusion.puddle,0)),0,1);local surfaceF=clamp(math.max(num(fusion.surface,0),num(rainDir.surface,0)),0,1)
 ...
 local fusionSignal=clamp(num(fusion.signal,num(rainDir.signal,0)),0,1);local fusionPriority=clamp(num(fusion.priority,0),0,1);local fusionContrast=clamp(num(fusion.contrast,1),1,1.13)
 ...
 local distant=clamp(num(p.distantSize,1.05)*lightWeight+nightF*0.10+reflectionF*0.03+sceneLight*0.06+nightUrban*0.04+emergencyF*0.02+fusionSignal*0.04+fusionPriority*0.03,0.90,1.20)
 local reflection=clamp(num(p.distantReflection,0.85)*lightWeight+wetF*0.10+stormF*0.02+reflectionF*0.06+vehicleWet*0.02+fusionSignal*0.05+fusionPriority*0.03+surfaceF*0.02+puddleF*0.02,0.70,1.00)
 local upscale=clamp(num(p.distantUpscale,1.50)*lightWeight+nightF*0.10+reflectionF*0.04+fusionSignal*0.03,1.10,1.80)
 valid('distantlights.size',distant);valid('distantlights.sizeReflections',reflection);valid('distantlights.sizeUpscale',upscale);valid('distantlights.sizeUpscaleReflections',clamp(upscale+wetF*0.05+reflectionF*0.03+fusionSignal*0.02+puddleF*0.02,1.10,1.80))
 local rain=num(p.rain,3500)+stormF*650+rainF*250+surfaceF*120;valid('rain.NumberParticles',clamp(rain*rainWeight,2800,5500));valid('rain.UseLitShader',1.0);valid('rain.diffuse',clamp(0.90+wetF*0.10+stormF*0.02+fusionContrast*0.01-0.01,0.90,1));valid('rain.ambient',clamp(0.35+wetF*(0.08+stormF*0.03),0.35,0.46));valid('rain.wrapScale',0.60);valid('rain.wrapBias',0.40);valid('rain.defaultlight.red',1.0);valid('rain.defaultlight.green',1.0);valid('rain.defaultlight.blue',1.0)
 ...
 local head=1.45+nightF*0.16+emergencyF*0.08+sceneLight*0.05+num(nightCore.headlight,0)*0.04+fusionSignal*0.05+fusionPriority*0.03;head=head*(0.96+0.04*lightWeight);valid('car.headlight.HDRIntensity',clamp(head,1.35,1.74));valid('car.headlight.fullbeam.HDRIntensity',clamp(1.15+nightF*0.06+wetF*0.02+num(nightCore.headlight,0)*0.02+fusionSignal*0.01,1.15,1.22))
 Apply.lastProfile=name;Apply.lastMultiplier=mult;Apply.lastStateKey=table.concat({scene.phase or s.phase or '?',scene.weather or s.weather or '?',string.format('night%.2f',nightF),string.format('rain%.2f',rainF),string.format('wet%.2f',wetF),string.format('surface%.2f',surfaceF),string.format('puddle%.2f',puddleF),string.format('urban%.2f',nightUrban),string.format('em%.2f',emergencyF),string.format('fusion%.2f',fusionSignal),name,budget.tier or '?'},'|');return true
end
