...
-- Weather Finish alimenta somente chaves já validadas.
...
local function apply()
 ...
 local wetF=clamp(tonumber(finish.wet) or tonumber(trans.wet) or tonumber(wf.wet) or 0,0,1)
 local stormF=clamp(tonumber(finish.storm) or tonumber(trans.storm) or tonumber(wf.storm) or 0,0,1)
 local fogF=clamp(tonumber(wf.fog) or 0,0,1);local skyF=clamp(tonumber(wf.sky) or 0,0,1);local reflectionF=clamp(tonumber(wf.reflection) or 0,0,1)
 local emergencyF=clamp(tonumber(finish.emergency) or 0,0,1);local interior=s.interior==true;local night=s.phase=='night';local wet=s.wet==true;local storm=s.weather=='THUNDER'
 local distant=clamp((p.distantSize or 1.05)*mult+nightF*0.08+skyF*0.015,0.90,1.20)
 ...
 Apply.lastProfile=name;Apply.lastMultiplier=mult;Apply.lastStateKey=table.concat({s.phase or '?',s.weather or '?',tostring(wet),tostring(interior),tostring(s.emergency==true),name,budget.tier or '?',string.format('fog%.2f',fogF),string.format('sky%.2f',skyF)},'|');return true
end
