...
-- Todos os novos diretores entregam pesos; nenhuma chave especulativa é escrita.
...
local function apply()
 ...
 local budget=call('GetVisualBudget') or {multiplier=1.0,tier='NATIVE'};local adapter=call('GetVisualBudgetAdapter') or {};local gov=call('GetVisualLightGovernorState') or {};local mult=clamp(tonumber(adapter.multiplier) or tonumber(budget.multiplier) or 1.0,0.72,1.0)
 ...
 local nightF=clamp(tonumber(comp.night) or tonumber(finish.night) or tonumber(scene.night) or tonumber(ctx.night) or 0,0,1);local wetF=clamp(tonumber(comp.wet) or tonumber(finish.wet) or tonumber(scene.wet) or tonumber(wf.wet) or 0,0,1);local stormF=clamp(tonumber(comp.storm) or tonumber(finish.storm) or tonumber(scene.storm) or tonumber(wf.storm) or 0,0,1);local reflectionF=clamp(tonumber(comp.reflection) or tonumber(wf.reflection) or tonumber(scene.reflectionDemand) or 0,0,1);local sceneLight=clamp(tonumber(comp.street) or tonumber(scene.streetDemand) or tonumber(lb.street) or 0,0,1);local emergencyF=clamp(tonumber(comp.emergency) or tonumber(e.active) or tonumber(finish.emergency) or 0,0,1);local interior=s.interior==true or scene.interior>0.5;local solar=clamp(tonumber(comp.solar) or tonumber(sun.solar) or 1,0.5,1.08);local exposure=clamp(tonumber(comp.exposure) or tonumber(ex.factor) or 1,0.88,1.04);local balance=clamp(tonumber(comp.balance) or tonumber(lb.balance) or 1,0.90,1.04);local governor=clamp(tonumber(gov.boost) or 1,0.72,1.18)
 local lightWeight=clamp((tonumber(comp.lightWeight) or mult)*balance*exposure*clamp(governor,0.78,1.08),0.72,1.0);local rainWeight=clamp(tonumber(comp.rainWeight) or mult,0.72,1.0)
 ...
 Apply.lastProfile=name;Apply.lastMultiplier=mult;Apply.lastStateKey=table.concat({scene.phase or s.phase or '?',scene.weather or s.weather or '?',string.format('wet%.2f',wetF),string.format('solar%.2f',solar),string.format('em%.2f',emergencyF),string.format('exp%.2f',exposure),string.format('bal%.2f',balance),string.format('gov%.2f',governor),tostring(interior),name,budget.tier or '?'},'|');return true
end
